<template>
  <div class="demo_page" :style="{minHeight:windowHeight1}">
    <div class="top_card">
      <div class="card_tab" @tap="changeCard" v-if="JSON.stringify(carArrFirst) != '{}'">
        ...
        <div style="display:flex;flex-direction:column;align-items:center;">
          <img
            src="https://m.aerp.com.cn/mini-app-main/changeCarIcon.png"
            alt
    ...
    <div class="box" v-for="(item1,index1) in boxarr" :key="index1">
      ...
      <div class="bottom_box" v-for="(itema,indexa) in item1.packageItems" :key="indexa">
        <!-- 小保养服务 -->
        <div class="byservice" @tap="serviceClick(itema,indexa,item1,index1)">
          <div class="byservice_left">
            <img :src="itema.imageUrl" alt mode="aspectFit" />
            <div style="margin-left:10rpx;">
              <p
                style="font-size:30rpx;font-family:Source Han Sans CN;font-weight:bold;color:rgba(51,51,51,1);"
              >{{itema.zhName}}</p>
              <p
                style="font-size:24rpx;font-family:Source Han Sans CN;font-weight:400;color:rgba(166,165,174,1);margin-top:14rpx;"
              >{{itema.briefDescription}}</p>
            </div>
          </div>
        ...
        <div v-if="itema.items.length > 0 && itema.isDefaultExpand">
          <div v-for="(itemaa1,indexaa1) in itema.items">
            <div
              :class="['set_meal',{'set_meal_border':itemb.isDefaultSelect}]"
              v-for="(itemb,indexb) in itemaa1.products"
              :key="indexb"
              v-if="itemaa1.products.length>0"
            >
              ...
              <div :class="['set_background',{'set_background1':itemb.isDefaultSelect}]">
                ...
                <div>
                  <div
                    @tap="commodityClcik(itemb,indexb)"
                    style="width:480rpx;display:flex;flex-direction:column;"
                  >
                    <div>
                      <p class="p_one">
                        <text
                          style="font-size:28rpx;font-family:Source Han Sans CN;font-weight:400;color:rgba(51,51,51,1);"
                  ...
                  <p class="p_four">
                    ...
                    <van-stepper
                      ...
                      :data-indexa="indexa"
                      :data-index1="index1"
                      :data-indexaa1="indexaa1"
                    />
                  </p>
                </div>
                <div
                  class="byservice_right"
                  @tap="selectCkick(itemb, indexa,boxarr ,indexb)"
                  style="position:absolute;right:-10rpx;top: 15rpx"
                >
                  <img
            ...
            <div
              class="showallshop"
              v-if="itemaa1.products.length"
              @tap="showallshop(itema,index1,indexa,indexaa1)"
            >{{itema.isShowAllPackage?'收起全部套餐':'展开全部套餐'}}</div>
          </div>
        </div>
        <!-- 暂无合适商品推荐 -->
    ...
    <div class="backimg" v-if="boxarr.length>0">
      <img
        src="https://m.aerp.com.cn/mini-app-main/process.png"
        alt
        style="width:100%;height:290rpx;margin-top:20rpx;"
        mode="aspectFit"
      />
      <img
        src="https://m.aerp.com.cn/mini-app-main/advantage.png"
        alt
        style="width:100%;height:290rpx;margin-top:20rpx;"
        mode="aspectFit"
      />
    ...
    <div class="bottom" style="z-index:30">
      <img
        src="https://m.aerp.com.cn/mini-app-main/maintenance_tel_icon.png"
        alt
        class="tel"
        @tap="telClick"
        mode="aspectFit"
      ...
      <div class="orange" @tap="downUp">
        明细
        <img
          src="https://m.aerp.com.cn/mini-app-main/downRed.png"
          alt
          class="down"
          v-if="downShow"
          mode="aspectFit"
        />
        <img
          src="https://m.aerp.com.cn/mini-app-main/upRed.png"
          alt
          class="down"
          v-else
          mode="aspectFit"
...
export default {
  ...
  methods: {
          ...
          if (res.message) {
            ...
            listarr.forEach(item => {
              item.packageItems.forEach(resa => {
                ...
                // resa.items[0].products.push(addData)
    ...
    sureJiesuan() {
      if (this.cardtitle == '') {
        ...
        wx.showToast({ title: '暂无上门服务门店', icon: 'none' })
    ...
    getPageDetail() {
      let that = this
      console.log(111, this.$store.state.curCityInfo.city)
      if (this.$store.state.carArr[0]) {
        if (this.$store.state.curCityInfo.city) {
        } else {
          wx.showModal({
  ...
  onShow() {
    console.log(this.globalData.isCarShow, 'this.isShow')
    // eventBus.$on("isonShow1", res => {
    //   console.log(res);
    //   this.isShow = res;
    // });
