...
namespace Ae.BasicData.Service.Common.Extension
{
    public static class StringExtension
    {
        ...
        public static string GetStringSpecificLength(this string value, int specificLength = 3000)
        {
            var tmp = Encoding.Default.GetBytes(value ?? "");
            return tmp.Length <= specificLength 
                ? value
                : Encoding.Default.GetString(tmp.Take(specificLength - 1).ToArray());
        }
