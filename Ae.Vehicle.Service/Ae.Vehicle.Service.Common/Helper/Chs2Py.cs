...
namespace Ae.Vehicle.Service.Common.Helper
{
    ...
    public static class Chs2Py
    {
        ...
        private static readonly Regex Reg = new Regex("^[\u4e00-\u9fa5]$", RegexOptions.Compiled);//验证是否输入汉字
#if NETFRAMEWORK
        ...
        public static string Pinyin(this string chstr)
        {
            if (!string.IsNullOrWhiteSpace(chstr))
                try
                {
                    ...
                    for (var j = 0; j < mChar.Length; j++)
                    {
                        ...
                        {
                            arr = DefaultEncoding.GetBytes(mChar[j].ToString());
                            m1 = arr[0];
                            m2 = arr[1];
                            asc = m1 * 256 + m2 - 65536;
                            if (asc > 0 && asc < 160)
