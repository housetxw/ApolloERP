          ...
          <!--   <el-table-column
            ...
            <el-table-column width="200px" fixed="right">
              <template slot="header">
                <label>操作</label>
                <el-button type="text" size="small" @click="allconfirm()"
                  >总部全部确认</el-button
                >
              </template>
              <template slot-scope="scope">
                <el-button
                  type="text"
                  size="small"
                  :disabled="scope.row.rgCheckResult != '' ? true : false"
                  @click="rgConfirm(scope.row)"
                  >总部确认</el-button
                >
                <el-button
                  type="text"
...
export default {
  ...
  data() {
    return {
      ...
      rgConfirmCondtion: {
        orderNos: [],
        rgCheckResult: undefined,
        locationId: undefined,
      },
      rgConfirmCondtionInit: {
        orderNos: [],
        rgCheckResult: undefined,
        locationId: undefined,
      },
  ...
  methods: {
        ...
        this.$confirm("确定操作吗！", "警告", {
          ...
          .then(() => {
            for (
              ...
              this.rgConfirmCondtion.orderNos.push(
                this.$refs.multipleTable.selection[i].orderNo
              );
            }
            this.rgConfirmCondtion.locationId = this.condition.locationId;
            this.rgConfirmCondtion.rgCheckResult = "已核对";
            var vm = this;
            console.log("formModel: " + JSON.stringify(this.rgConfirmCondtion));

            appSvc
              .updateRgAccountCheckResult(this.rgConfirmCondtion)
              .then(
                (res) => {
                  if (res.code == 10000) {
                    this.$message({
                    ...
                    this.rgConfirmCondtion = JSON.parse(
                      JSON.stringify(this.rgConfirmCondtionInit)
                    );
                  } else {
                    this.$message({
                      message: res.message,
    ...
    rgConfirm(row) {
      this.$confirm("确定操作吗！", "警告", {
        ...
        .then(() => {
          this.rgConfirmCondtion.locationId = this.condition.locationId;
          this.rgConfirmCondtion.orderNos.push(row.orderNo);
          this.rgConfirmCondtion.rgCheckResult = "已核对";
          var vm = this;
          console.log("formModel: " + JSON.stringify(this.rgConfirmCondtion));
    ...
    handleSelectionChange(val) {
      ...
      this.$refs.multipleTable.selection = val;
    },
    ...
    fetchData() {
      ...
      appSvc
        .getShopAccountChecks(this.condition)
        .then(
          (res) => {
            var data = res.data;
