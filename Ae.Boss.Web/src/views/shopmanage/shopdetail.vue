  ...
  <main class="container">
    ...
    <section id="indexPage" style="margin-top: 20px">
      <el-form :model="formModel" :inline="true" ref="formModel">
        <router-link
          :to="{ name: 'shopservicetypeindex', query: { shopId: id } }"
        >
          ...
          <el-main>
            ...
            >
              ...
              <el-row :gutter="20">
                ...
                >
                  <el-radio-group v-model="shop.isCreateAccount">
                ...
                <el-form-item
                  ...
                  prop="isSendMessage"
                >
                  <el-radio-group v-model="shop.isSendMessage">
...
export default {
  ...
  methods: {
    ...
    regenerateCode() {
      this.generateCodeCondition.shopId = this.id;

      shopManageSvc
        .reGenerateAppletCode(this.generateCodeCondition)
        .then(
          (res) => {
            if (res.code == 10000) {
              this.$message({
            ...
            if (res.data != "") {
              ...
              this.uploadFileModel.name = "快速排队码";
              this.uploadFileModel.url = res.data[0];
              this.tempfileList7.push(this.uploadFileModel);

              this.uploadFileModel = JSON.parse(
                JSON.stringify(this.uploadFileModelInit)
              );
              this.uploadFileModel.name = "门店信息码";
              this.uploadFileModel.url = res.data[1];
              this.tempfileList7.push(this.uploadFileModel);
            }
            this.generateCodeCondition.shopId = undefined;
          },
          (error) => {
            console.log(error);
          }
      ...
      if (this.statusFormModel.stopTime && this.statusFormModel.stopTime.length > 0) {
        this.$confirm("确定操作吗！", "警告", {
          ...
          .then(() => {
            ...
            //this.statusFormModel.stopTime=[];
            this.modifySuspendTimeCondition.shopId = this.id;
            this.modifySuspendTimeCondition.times =
              this.statusFormModel.stopTime;
            shopManageSvc
              .modifyShopSuspendTime(this.modifySuspendTimeCondition)
              .then(
                (res) => {
                  if (res.code == 10000) {
                    this.$message({
                  ...
                  this.modifySuspendTimeCondition.times = [];
                  this.modifySuspendTimeCondition.shopId = undefined;
                  //this.statusFormModel.stopTime=[];
                },
                (error) => {
                  console.log(error);
    ...
    changeStatus() {
      ...
      if (this.shop.status == 2) {
        ...
        //将时间清空 设置默认时间
        this.modifySuspendTimeCondition.shopId = this.id;
        this.modifySuspendTimeCondition.times.push("1900-01-01 00:00:00");
        this.modifySuspendTimeCondition.times.push("1900-01-01 00:00:00");
        shopManageSvc
          .modifyShopSuspendTime(this.modifySuspendTimeCondition)
          .then(
    ...
    getCompanyList() {
      shopManageSvc
        ...
        .then(
          (res) => {
            debugger;
            let itemsData = JSON.parse(JSON.stringify(res.data));
            console.log("itemData" + itemsData.length);
            debugger;
            ...
            console.log("codelist" + this.categoryCodeList);
          },
          (error) => {
            console.log(error);
          }
        ...
        .then(
          (res) => {
            ...
            shopManageSvc
              ...
              .then(
                (res) => {
                  // this.shop.serviceRegion = "";
                  //  debugger;
                  this.shopServiceRegion = res.data;

                  debugger;
                  let shopServiceRegions = "";
                  if (this.shopServiceRegion.regions != undefined) {
                    for (
                      var i = 0;
                      i < this.shopServiceRegion.regions.length;
                      i++
                    ) {
                      shopServiceRegions =
                        shopServiceRegions +
                        this.shopServiceRegion.regions[i].district +
                        ";";
                    }
                  }

                  this.shop.serviceRegion = shopServiceRegions;

                  console.log("serviceRegion=", shopServiceRegions);
                },
                (error) => {
                  console.log(error);
                }
            ...
            if (this.shop.appletCode != "") {
              this.uploadFileModel = JSON.parse(
                JSON.stringify(this.uploadFileModelInit)
              );
              this.uploadFileModel.name = "快速排队码";
              this.uploadFileModel.url = this.shop.appletCode;
              this.tempfileList7.push(this.uploadFileModel);
            }

            if (this.shop.shopAppletCode != "") {
              this.uploadFileModel = JSON.parse(
                JSON.stringify(this.uploadFileModelInit)
              );
              this.uploadFileModel.name = "门店信息码";
              this.uploadFileModel.url = this.shop.shopAppletCode;
              this.tempfileList7.push(this.uploadFileModel);
            }
          ...
          .then(
            (res) => {
              ...
              for (var i = 0; i < this.tempfileList1.length; i++) {
                if (this.tempfileList1[i].imgId == this.deleteCondition.imgId) {
          ...
          .then(
            (res) => {
              ...
              for (var i = 0; i < this.tempfileList2.length; i++) {
                if (this.tempfileList2[i].imgId == this.deleteCondition.imgId) {
          ...
          .then(
            (res) => {
              ...
              for (var i = 0; i < this.tempfileList3.length; i++) {
                if (this.tempfileList3[i].imgId == this.deleteCondition.imgId) {
          ...
          .then(
            (res) => {
              ...
              for (var i = 0; i < this.tempfileList4.length; i++) {
                if (this.tempfileList4[i].imgId == this.deleteCondition.imgId) {
          ...
          .then(
            (res) => {
              ...
              for (var i = 0; i < this.tempfileList5.length; i++) {
                if (this.tempfileList5[i].imgId == this.deleteCondition.imgId) {
          ...
          .then(
            (res) => {
              ...
              for (var i = 0; i < this.tempfileList6.length; i++) {
                if (this.tempfileList6[i].imgId == this.deleteCondition.imgId) {
        ...
        if (valid) {
          shopManageSvc
            .ModifyShopConfigExpense({
              ...
              DailyTireOrderUpperLimit:
                this.shopConfig.dailyTireOrderUpperLimit,
              UpdateBy: "",
            })
            .then(
              (res) => {
    ...
    ModifyShopBankAccount() {
      shopManageSvc
        .ModifyShopBankAccount({
          ...
          OpeningBranch: this.shopBankCard.openingBranch,
