  ...
  <main class="container">
    ...
    >
      ...
      <template v-slot:condition>
        ...
        <el-form-item>
          <el-select v-model="condition.status" clearable placeholder="请选择状态" style="width:150px">
            <el-option
        ...
        <el-form-item label v-if="false">
          <el-select v-model="condition.type" placeholder="请选择货物来源" style="width:150px">
            <el-option
      ...
      <template v-slot:tb_cols>
        <el-table-column type="expand">
          <template slot-scope="props">
            <el-table border :data="props.row.products" style="width: 100%" size="mini">
              ...
              <rg-table-column label="领取数量" prop="num" is-number fix-min />
              <rg-table-column label="订单耗用" prop="installNum" is-number fix-min />
              <rg-table-column label="已还" prop="actualNum" is-number fix-min />
              <rg-table-column label="货损" prop="exceptionNum" is-number fix-min>
                <template slot-scope="scope">
                  <font
                    v-if="scope.row.exceptionNum>0"
                    style="color:red;"
                  >{{scope.row.exceptionNum}}</font>
              ...
              <rg-table-column label="待还" prop="receiveNum" is-number fix-min />
            </el-table>
          </template>
        </el-table-column>
        <!-- Table数据 -->
        ...
        <rg-table-column label="领用编号" prop="id" width="150"></rg-table-column>
        <rg-table-column label="领用技师" prop="techName"></rg-table-column>
        <rg-table-column label="领用日期" prop="receiveTime" is-datetime fix-min></rg-table-column>
    ...
    <section id="createOrUpdate">
      <rg-dialog
        ...
        :btnCancel="{label:'关闭',click:(done)=>{productcancel('productformModel')}}"
        :btnRemove="false"
        :footbar="true"
        width="100%"
        maxWidth="800px"
      ...
      >
        <template v-slot:content>
          <el-form :model="productformModel" :inline="true" ref="productformModel">
            <el-form-item>
              <el-input
                size="mini"
                placeholder="请输入产品名称"
        ...
        <template v-slot:footer>
          <el-button icon="el-icon-check" size="mini" type="primary" @click="saveProduct()">确定</el-button>
        </template>
      </rg-dialog>
    </section>
    <!-- 选择产品页面 -->
      ...
      >
        <template v-slot:content>
          <el-form :model="formModel" :rules="rules1" ref="formModel">
            ...
            <el-form-item label="出库时间" :label-width="formLabelWidth" prop="receiveTime">
              <el-date-picker
            ...
            <el-form-item>
              <el-button type="primary" size="mini" @click="selectProduct()">选择产品</el-button>
            </el-form-item>
          </el-form>
  ...
  methods: {
            ...
            if (valid) {
              ...
              if (this.productStockTable.length > 0) {
                ...
                if (isValid) {
                  ...
                  appSvc
                    .createHomeCareRecord(this.formModel)
                    .then(
                      res => {
                        if (res.code == 10000) {
                          this.$message({
    ...
    expandSelect(row, expandedRows) {
      if (expandedRows.length != 0) {
        ...
        appSvc
          .getHomeCareProducts(this.careproductCondition)
          .then(
            res => {
              //  this.tableData = res.data;
              var data = res.data;
    ...
    fetchData() {
      this.getShopTechs();
      appSvc
        .getHomeCareRecordPages(this.condition)
        .then(
          res => {
            //  this.tableData = res.data;
            var data = res.data;
