  ...
  <main class="container">
    ...
    >
      ...
      <template v-slot:tb_cols>
        <rg-table-column label="门店编号" prop="id" />
        <rg-table-column label="门店简称" prop="simpleName" />
        <!-- <rg-table-column label="门店名称" prop="fullName" /> -->
        <rg-table-column label="公司名称" prop="shopCompanyName" />
        <rg-table-column label="地址" prop="address" />
        <rg-table-column label="经营状态" prop="status">
        ...
        <rg-table-column label="门店老板" prop="owner" fix-width="true">
          <template slot-scope="scope"
            >{{ scope.row.ownerName }}
            {{ scope.row.ownerPhone ? +scope.row.ownerPhone : "" }}</template
          >
        </rg-table-column>
        <rg-table-column label="门店类型" prop="type" />
        <rg-table-column label="状态" prop="status" align="center">
          <template slot-scope="scope">{{
            scope.row.online == 0 ? "下架" : "上架"
          }}</template>
        </rg-table-column>
        <rg-table-column label="操作" fixed="right" align="center" width="90">
          <template slot-scope="scope">
            <el-button-group>
          ...
          >
            ...
            <!-- <el-form-item label="门店全称" prop="fullName">
              <el-col :span="12">
                ...
                <el-input v-else v-model="shop.fullName" placeholder="门店全称" clearable></el-input>
              </el-col>
            </el-form-item> -->
            <el-form-item label="门店名称" prop="simpleName">
              <el-col :span="12">
            ...
            <el-form-item label="门店管理员" required>
              <el-col :span="6">
              ...
              <!-- <el-col :span="12">
                <el-checkbox
                  v-model="shopConfig.isCreateAccount"
                  style="margin-left: 5px"
                  >是否同步创建老板账号（短信自动发送账号信息）</el-checkbox
                >
              </el-col> -->
            </el-form-item>
            <!-- <el-form-item label="主管" prop="charge">
              <el-col :span="6">
                <el-select style="width:100%" v-model="shop.chargePhone">
                  <el-option
            ...
            <el-form-item label="专修品牌">
              <el-tag
                ...
                >{{ tag }}</el-tag
              >
              <el-button type="text" @click="addBrand()"
                >添加专修品牌</el-button
              >
            </el-form-item>
            ...
            <el-form-item label="正式上线日期" required>
              <el-col :span="6">
                <el-date-picker
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        // charge: [{ required: true, message: "请输入主管", trigger: "blur" }],
        longitude: [
          {
            validator: longreg,
            required: true,
  ...
  methods: {
    fetchData() {
      ...
      this.getEmployeeListByOrgIdAndType();
    },
    ...
    cancel(formName) {
      ...
      if (this.$refs[frmName]) this.$refs[frmName].resetFields();
    },
    resetForm() {
      (this.formModel = {}),
        (this.shop = {
    ...
    add() {
      ...
      this.shop.companyId = this.userInfo.organizationId;
      this.shop.shopCompanyName = this.userInfo.organizationName;
    },
    edit(sel) {
      this.formTitle = "编辑";
      this.formVisible = true;
      this.getShopById(sel.id);
    },
      ...
      shopManageSvc
        ...
        .then(
          (res) => {
            ...
            this.getShopServiceBrands(id); //已选择的品牌
        ...
        .then((res) => {
          ...
          upload(
            ...
            (complete) => {
              ...
              const key = complete.key;
              var url = host + "/" + key;
              this.shopBankCard.openingLicence = key;
            }
          );
        })
        .catch(() => {})
    ...
    save(shop) {
      ...
      let objEmployee = {};
      objEmployee = this.employeeList.find((item) => {
        //model就是上面的数据源
        return item.mobile === this.shop.chargePhone; //筛选出匹配数据
      });
      ...
      //   return false;
      // }
      // this.shop.charge = objEmployee.name;
      ...
      this.shopConfig.is_send_message = this.shopConfig.isCreateAccount;
      this.shop.fullName = this.shop.simpleName;
      debugger;
      this.shop.shopCompanyName = this.userInfo.organizationName;
      var shopData = {
        Shop: this.shop,
      ...
      console.log("shopData" + JSON.stringify(this.shop));
      shopManageSvc
    ...
    getEmployeeListByOrgIdAndType() {
      ...
      empSvc
        .GetEmployeeListByOrgIdAndType(employeeCondition)
        .then(
          (res) => {
    ...
    getShopServiceBrands(id) {
      this.brandCondition.shopId = id;
      shopManageSvc
